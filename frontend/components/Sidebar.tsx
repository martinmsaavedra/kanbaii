...
import { Plus, Trash2, Archive, FolderOpen, Pin, BarChart3, Settings } from 'lucide-react';
...
export function Sidebar() {
  const { projects, fetchProjects, setActiveSlug, deleteProject, updateProject } = useProjectStore();
  ...
  const handleDelete = async (e: React.MouseEvent, project: Project) => {
    ...
    try {
      ...
      if (projectSlug === project.slug) goHome();
      addToast(`${project.title} deleted`, 'success');
              ...
              return (
                <div key={project.id} className="relative mb-0.5 group">
                    ...
                    <div
                      ...
                      style={{
                        ...
                        ...(!isRunning ? {
                          ...
                          boxShadow: isActive ? `0 0 8px ${project.color}50` : 'none',
                        } : {}),
                    ...
                    {isExpanded && (
                      <div className="flex-1 min-w-0">
                        ...
                        {(project as any).workingDir ? (
                          <div className="text-label flex items-center gap-1 mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis text-text-muted">
                  ...
                  {menuOpen === project.slug && isExpanded && (
                    ...
                    >
                      <button
                        className="w-full py-2 px-3 flex items-center gap-2 bg-transparent text-text-secondary text-xs text-left
                                   transition-colors duration-100 hover:bg-surface-hover"
                        onClick={(e) => {
                          e.stopPropagation(); setMenuOpen(null);
                          setFolderModal({ slug: project.slug, current: (project as any).workingDir || '' });
                        }}
                      >
                      ...
                      <button
                        className={`w-full py-2 px-3 flex items-center gap-2 text-xs text-left transition-colors duration-100
                                    ${confirmDelete === project.slug
                                      ...
                                      : 'text-danger bg-transparent hover:bg-danger-dim'
                                    }`}
                        onClick={(e) => {
                          e.stopPropagation();
                          if (confirmDelete === project.slug) { setMenuOpen(null); handleDelete(e, project); }
                          else { setConfirmDelete(project.slug); setTimeout(() => setConfirmDelete(null), 3000); }
                        }}
                      >
                        <Trash2 size={14} /> {confirmDelete === project.slug ? 'Confirm Delete?' : 'Move to Trash'}
                      </button>
                    </div>
                  )}
                </div>
